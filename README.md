...
We drew inspiration from the Transfer Learning to Pytorch Tutorials provided on the course website to get a better understanding of working with PyTorch in Google Colab. For example, after reading about how the dataset definition worked, we decided to do something similar in the process of connecting our dataset to PyTorch. Our train was essentially the same as any train method, consisting of getting a batch, running the model forward, calculating loss, running the model backward, and finally updating.  
